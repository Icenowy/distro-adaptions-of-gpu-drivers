...
\begin{frame}
    ...
    额外的设备节点接口设计不合理的话，可能成为普通用户可以访问的安全漏洞。\\
\end{frame}
...
\begin{frame}
    ...
    然而很不幸的是，随着 Mesa 之外的 DRI 接口使用逐渐消亡，DRI 接口也逐渐转向 Mesa 内部接口，并被破坏性的简化；而且 DRI 接口从未得到妥善的文档。\\
    也就是说，随着发行版对 Mesa 的更新，使用 DRI 接口是一种未定义且将来很可能失效的操作。
\end{frame}
